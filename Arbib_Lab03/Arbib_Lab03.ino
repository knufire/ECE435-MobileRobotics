...
void wallBang() {
  ...
  switch (state) {
    case (FOLLOW_RIGHT):
      ...
      /*THIS DOESN'T WORK*/
      else if (ri_cerror > 0) {          //positive error means too close
        Serial.println("\trt wall: too close turn left");
        Serial.print("\t\tspin");
        spin(eighth_rotation, 1);              //turn left
        delay(100);
        forward(0.25);
        Serial.print("\t\t\tpivot");
        //pivot(half_rotation, 1);      //pivot left
        delay(100);
        Serial.print("\t\tpivot");
//        pivot(half_rotation, 1);   //pivot right to straighten up
        delay(100);
      }
      else if (ri_cerror < 0) {     //negative error means too far
        Serial.println("\trt wall: too far turn right");
        Serial.print("\t\tspin");
        spin(eighth_rotation, 0);              //turn left
        delay(100);
        forward(0.25);
        ...
        delay(100);
        Serial.print("\t\tpivot");
//        pivot(half_rotation, 0);   //pivot right to straighten up
        delay(100);
      }
      //}
      break;
